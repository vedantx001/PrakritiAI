...
import { buildAdminAvatarUrl } from '../../../utils/avatar';

const AdminNavbar = ({ isSidebarOpen, toggleSidebar, isDarkMode, toggleTheme, onProfileClick }) => {
  const { user } = useAuth();
  ...
  const avatarUrl = useMemo(() => buildAdminAvatarUrl({ name: displayName }), [displayName]);
  ...
  return (
    <header className="h-16 sticky top-0 z-40 bg-[var(--bg-card)]/80 backdrop-blur-md border-b border-[var(--border-color)] flex items-center justify-between px-6">
      <div className="flex items-center gap-4">
        ...
        <h2 className="text-xl font-bold text-[var(--text-main)]">Admin Dashboard</h2>
      </div>
