...
const Navbar = () => {
  ...
  return (
    <>
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 flex justify-between items-center gap-3">
          ...
          <Link to="/" className="flex items-center gap-3 group isolate">
            <img
              ...
              alt="PrakritiAI"
              className="h-12 sm:h-14 md:h-16 w-auto max-w-[220px] object-contain bg-transparent"
            />

            <span className="sr-only text-2xl md:text-3xl font-bold tracking-tight text-[var(--text-main)] leading-none group-hover:text-emerald-700 transition-colors">
      ...
      <div
        className={`lg:hidden fixed inset-0 z-[60] bg-black/40 backdrop-blur-sm transition-opacity duration-300 ${isMobileMenuOpen ? 'opacity-100 pointer-events-auto' : 'opacity-0 pointer-events-none'
          }`}
      ...
      {/* --- Mobile Menu Drawer --- */}
      <div
        className={`lg:hidden fixed top-0 right-0 w-[85vw] max-w-[380px] h-[100dvh] z-[70] bg-[var(--bg-primary)] shadow-[-15px_0_40px_-15px_rgba(0,0,0,0.3)] flex flex-col transition-transform duration-300 ease-[cubic-bezier(0.33,1,0.68,1)] ${isMobileMenuOpen ? 'translate-x-0' : 'translate-x-full'
          }`}
      >
        {/* Drawer Header */}
        <div className="flex items-center justify-between p-5 border-b border-[var(--border-color)]">
          <Link to="/" className="flex items-center gap-3" onClick={() => setIsMobileMenuOpen(false)}>
            ...
            />
            <span className="sr-only text-xl font-bold tracking-tight text-[var(--text-main)] ml-1">
              Prakriti<span className="text-emerald-600">AI</span>
            </span>
          </Link>
          <button
